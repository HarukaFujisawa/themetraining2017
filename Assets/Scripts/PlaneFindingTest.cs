    ...
    public class PlaneFindingTest : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            foreach (MeshFilter mesh in GetComponentsInChildren<MeshFilter>())
            {
                ...
                //written by fujisawa
                meshObject.Add(mesh.gameObject);
            }
        ...
        //written by fujisawa
        //meshObjectをplaneに置き換える
#endif

        private void replaceMeshToPlane()
        {
            //オリジナルのmeshObjectを非表示にする
            ...
            for (int i = 0; i < planes.Length; i++)
            {
                string planeName = "plane_" + i.ToString();
                //前のplaneを削除
                Destroy(GameObject.Find(planeName + "(Clone)"));
                ...
                GameObject plane = (GameObject)Resources.Load("Prefabs/plane");
                Instantiate(plane);
                plane.name = planeName;
                plane.transform.position = center;
                plane.transform.localScale = extents;
